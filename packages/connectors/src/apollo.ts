...
const SIGNAL_CONFIGS: Array<{
  ...
  },
  {
    ...
    offer: "webb_design",
    label: "B2B-tjänsteföretag med ofta föråldrade webbsajter → webb_design",
  },
];
...
// ------------------------------------------------------------
// SOURCE D — Swedish decision makers (VD/CEO/Grundare) at SMBs
// Uses people search instead of company search — much more reliable
// for Swedish market where company keyword data is sparse.
// Returns decision makers with Apollo ID ready for enrichment.
// FREE — no credits consumed.
// ------------------------------------------------------------
export type SwedishDecisionMaker = ProspectSignal & {
  vd_first_name: string;
  vd_last_name: string;
  apollo_person_id: string;
};

export async function searchSwedishDecisionMakers(opts: {
  apiKey: string;
  industryKeywords?: string[];
  employeeRange?: "small" | "mid" | "both";
  limit?: number;
  page?: number;
}): Promise<SwedishDecisionMaker[]> {
  const empRanges = opts.employeeRange === "small"
    ? ["5,9", "10,49"]
    : opts.employeeRange === "mid"
    ? ["50,199"]
    : ["10,49", "50,199"];
  ...
  const body: Record<string, unknown> = {
    ...
    organization_num_employees_ranges: empRanges,
    per_page: Math.min(opts.limit ?? 25, 100),
    page: opts.page ?? 1,
  };

  if (opts.industryKeywords?.length) {
    body["q_organization_keyword_tags"] = opts.industryKeywords;
  }

  const data = await apolloPost(opts.apiKey, "/mixed_people/api_search", body);
  const people = (data["people"] ?? []) as ApolloPerson[];

  const VC_NOISE = ["capital", "ventures", "invest", " fund", "equity", "accelerat", "holding", "asset"];

  return people
    .filter((p) => p.first_name && p.last_name && p.organization_name)
    .filter((p) => !VC_NOISE.some((s) => (p.organization_name ?? "").toLowerCase().includes(s)))
    .map((p): SwedishDecisionMaker => ({
      source: "digital_presence",
      company_name: p.organization_name ?? "",
      signals: [
        `Beslutsfattare: ${p.name ?? ""} (${p.title ?? ""})`,
        p.city ? `Ort: ${p.city}` : "",
      ].filter(Boolean),
      suggested_offer_hint: undefined,
      extra: {
      ...
      vd_first_name: p.first_name ?? "",
      vd_last_name: p.last_name ?? "",
      apollo_person_id: p.id ?? "",
    }));
}
